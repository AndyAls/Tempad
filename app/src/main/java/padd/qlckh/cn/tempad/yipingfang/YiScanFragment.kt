...
class YiScanFragment : BaseFragment() {
    ...
    private fun postData(rowBean: YiUserInfo.RowBean, weight: String, d: Double, status: String) {
        ...
        RxHttpUtils.createApi(ApiService::class.java)
            .postData(
                rowBean.fullname, rowBean.id, weight, rowBean.items, rowBean.phone,
    ...
    private fun initData() {
        ...
        tvDumpType.text = when (tagCheck) {
            ...
            YiMainActivity.BOLI -> {
                ...
                ivScanCode.setImageBitmap(
                    QRUtils.getInstance()
                        .createQRCode("{\"status\":\"2,3\",\"wyid\":\"" + AppUtils.getDeviceId(getActivity()) + "\",\"fenlei\":\"可回收/废玻璃\"}")
                )
                "玻璃"
            }
            ...
            YiMainActivity.SULIAO -> {
                ...
                ivScanCode.setImageBitmap(
                    QRUtils.getInstance()
                        .createQRCode("{\"status\":\"2,1\",\"wyid\":\"" + AppUtils.getDeviceId(getActivity()) + "\",\"fenlei\":\"可回收/塑料\"}")
                )
                "塑料"
            }
    ...
    private fun onHide() {
        ...
        handler.removeMessages(SCAN_WHAT)
